...
export async function runDailyBackup() {
	...
	try {
		...
		// swallow when table doesn't exist in ephemeral envs
		return { id: 'n/a', runAt, location: provider, pathOrDriveId: 'n/a', status: 'failed', message: e?.message } as any;
	}
}
...
export async function ensureDailyBackup() {
	try {
		...
		// if table missing or DB down, skip silently
		return { id: 'n/a', status: 'skipped' } as any;
	}
}